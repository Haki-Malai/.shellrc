...
- Output pattern:
  ...
  - `git ri` mirrors upstream output for fetch, checkout, fast-forward merge, and interactive rebase commands.
...
- Side effects:
  ...
  - `git ri` updates local refs from `origin`, may fast-forward local `main`, and may rewrite the original branch through interactive rebase.
